...
void khomichenko::readData(std::istream& in, Queue< std::string >& allData)
{
  while(!in.eof())
  {
...
void khomichenko::separateElements(std::string str, Queue< std::string >& queue)
{
  std::string temp = "";
...
void khomichenko::separateElements(Queue< std::string > oldQ, doubleQ& sepQ)
{
...
int khomichenko::getPriority(char c)
{
  if (c == '/' || c == '*' || c == '%')
  {
    return 2;
...
void khomichenko::getPostfix(Queue< std::string >& oldQ, Queue< std::string >& newQ)
{
  bool isCorrect = true;
...
void khomichenko::getPostfix(doubleQ oldQ, doubleQ& newQ)
{
  size_t size = oldQ.getSize();
  for(size_t i = 0; i < size; i++)
  {
...
long long khomichenko::getComputation(Queue< std::string > queue)
{
...
void khomichenko::getComputation(doubleQ data, Stack< long long >& result)
{
  long long outcome;
...
void khomichenko::output(std::ostream& out, Stack< long long > result)
{
