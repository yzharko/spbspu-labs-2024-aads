...
void panov::readFile(const std::string& path, BinarySearchTree< int, std::string >& bst)
{
  if (path.empty())
  {
  ...
  while (!file.eof())
  {
    ...
    panov::readDataSet(line, bst);
  }
  file.close();
}

int panov::parseNum(const std::string& num)
{
  int result = 0;
  result = std::stoi(num);
  return result;
...
void panov::readDataSet(const std::string& dataSet, BinarySearchTree< int, std::string >& bst)
{
  size_t pos = 0;
  std::string word;
  word.reserve(10);
