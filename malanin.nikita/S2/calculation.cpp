...
void malanin::readInfixForm(std::istream& input, Queue< std::string >& result)
{
...
void malanin::strToQueue(std::string expression, Queue< std::string >& result)
{
  std::string symbol = "";
  size_t i = 0;
...
bool malanin::isOperand(std::string symbol)
{
  for (char c : symbol)
  {
    if (!std::isdigit(c))
...
bool malanin::isOperator(std::string c)
{
  if (c == "/" || c == "*" || c == "+" || c == "-" || c == "%")
  {
    return true;
...
int malanin::precendence(std::string c)
{
  if (c == "/" || c == "*" || c == "%")
  {
    return 2;
...
void malanin::infixToPostfix(Queue< std::string >& expression, Queue< std::string >& result)
{
...
long long malanin::countOperation(long long fval, long long sval, std::string operation)
{
  long long result = 0;
...
long long malanin::calculateExpr(Queue< std::string >& expression)
{
...
void malanin::formResultStack(Queue< std::string >& expressions, Stack< long long >& resultStack)
{
