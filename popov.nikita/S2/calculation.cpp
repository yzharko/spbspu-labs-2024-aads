...
void popov::strToQueue(std::string expression, Queue< std::string >& result)
{
  std::string symbol = "";
  size_t i = 0;
...
bool popov::isOperand(std::string symbol)
{
  for (char c : symbol)
  {
    if (!std::isdigit(c))
...
bool popov::isOperator(std::string c)
{
  if (c == "/" || c == "*" || c == "+" || c == "-" || c == "%")
  {
    return true;
...
int popov::precendence(std::string c)
{
  if (c == "/" || c == "*" || c == "%")
  {
    return 2;
...
void popov::infixToPostfix(Queue< std::string >& expression, Queue< std::string >& result)
{
...
long long popov::countOperation(long long fval, long long sval, std::string operation)
{
  long long result = 0;
...
long long popov::calculateExpr(Queue< std::string >& expression)
{
...
void popov::formResultStack(Queue< std::string >& expressions, Stack< long long >& resultStack)
{
