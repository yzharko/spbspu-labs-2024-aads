...
bool maksimov::checkOperand(std::string symb)
{
  for (char c : symb)
  {
    if (std::isdigit(c))
...
bool maksimov::checkOperator(std::string symb)
{
...
int maksimov::priori(std::string symb)
{
  if (symb == "+" || symb == "-")
  {
    return 1;
...
void maksimov::readTerms(std::istream& in, Queue< std::string >& terms)
{
  std::string term;
  while (!in.eof())
  {
...
void maksimov::makeQueue(std::string strTerm, Queue< std::string >& queue)
{
  size_t termLen = 0;
  std::string symb = "";
...
void maksimov::postfixation(Queue< std::string >& infix, Queue< std::string >& postfix)
{
...
long long maksimov::makeOperation(long long first, long long second, std::string operation)
{
  long long res = 0;
  if (operation == "+")
  {
...
long long maksimov::calculateTerm(Queue< std::string >& term)
{
...
void maksimov::makeResults(Queue< std::string >& terms, Stack< long long >& results)
{
  std::string strTerm;
  long long res = 0;
