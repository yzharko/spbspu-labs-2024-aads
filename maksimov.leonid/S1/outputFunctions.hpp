...
using pair = maksimov::List< std::pair< std::string, maksimov::List< unsigned long long > > >;
...
template < typename T >
maksimov::List< T > maksimov::outputNums(pair& data, size_t& maxLenSeq, bool& overflowCheck)
{
...
template < typename T >
void maksimov::outputSums(List< T >& sums)
{
  if (sums.isEmpty())
  {
    std::cout << 0;
  ...
  {
    typename maksimov::List< T >::Iterator iteratorSums = sums.begin();
