...
void litsinger::readLine(std::istream & is, litsinger::Queue< litsinger::Element > & infix)
{
  std::string input;
...
bool litsinger::ifFirstPriority(litsinger::Element & element)
{
  return (element.elem_.operator_.operator_ == '+' or element.elem_.operator_.operator_ == '-');
}

bool litsinger::ifEquallyImportant(litsinger::Stack< litsinger::Element > & stack, litsinger::Element & element)
{
  while(!stack.empty())
  {
...
void litsinger::makePostfix(litsinger::Queue< litsinger::Element > & infix, litsinger::Postfix & postfix)
{
  Stack< Element > stack;
  while (!infix.empty())
  {
...
long long int litsinger::calculate(litsinger::Postfix & postfix)
{
