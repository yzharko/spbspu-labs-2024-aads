...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (isInt) {
      ...
      std::deque< int > deq;
      kaseev::generateNumbers(singleList, doubleList, deq, size, true);
      kaseev::performSorts(singleList, doubleList, deq, ascending, std::cout);
    }
    else {
      std::forward_list < float > singleList;
      std::list< float > doubleList;
      std::deque< float > deq;
      kaseev::generateNumbers(singleList, doubleList, deq, size, false);
      kaseev::performSorts(singleList, doubleList, deq, ascending, std::cout);
    }
  }
