...
void batunov::readInfixForm(std::istream& input, Queue< std::string >& result)
{
...
void batunov::strToQueue(std::string expression, Queue< std::string >& result)
{
  std::string symbol = "";
  size_t i = 0;
...
bool batunov::isOperand(std::string symbol)
{
  for (char c : symbol)
  {
    if (!std::isdigit(c))
...
bool batunov::isOperator(std::string c)
{
  if (c == "/" || c == "*" || c == "+" || c == "-" || c == "%")
  {
    return true;
...
int batunov::precendence(std::string c)
{
  if (c == "/" || c == "*" || c == "%")
  {
    return 2;
...
void batunov::infixToPostfix(Queue< std::string >& expression, Queue< std::string >& result)
{
...
long long batunov::countOperation(long long fval, long long sval, std::string operation)
{
  long long result = 0;
...
long long batunov::calculateExpr(Queue< std::string >& expression)
{
...
void batunov::formResultStack(Queue< std::string >& expressions, Stack< long long >& resultStack)
{
