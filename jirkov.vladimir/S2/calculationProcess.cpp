...
void jirkov::readInfixForm(std::istream& input, Queue< std::string >& result)
{
...
void jirkov::strToQueue(std::string expression, Queue< std::string >& result)
{
  std::string symbol = "";
  size_t i = 0;
...
bool jirkov::isOperand(std::string symbol)
{
  for (char c : symbol)
  {
    if (!std::isdigit(c))
...
bool jirkov::isOperator(std::string c)
{
  if (c == "/" || c == "*" || c == "+" || c == "-" || c == "%")
  {
    return true;
...
int jirkov::precendence(std::string c)
{
  if (c == "/" || c == "*" || c == "%")
  {
    return 2;
...
void jirkov::infixToPostfix(Queue< std::string >& expression, Queue< std::string >& result)
{
...
long long jirkov::countOperation(long long fval, long long sval, std::string operation)
{
  long long result = 0;
...
long long jirkov::calculateExpr(Queue< std::string >& expression)
{
...
void jirkov::formResultStack(Queue< std::string >& expressions, Stack< long long >& resultStack)
{
